...
asyncTest('Parser: parser freeze test for C mode', function() {
  const c = new C({
    knownFunctions: {
      puts: {color: 'blue'},
      printf: {color: 'blue'},
...
asyncTest('Parser: parser freeze test comment consolidation', function() {
  const c = new C({
    knownFunctions: {
      puts: {color: 'blue'},
      printf: {color: 'blue'},
...
asyncTest('Controller: ANTLR paren wrap rules', function() {
  let editor;
...
asyncTest('Controller: ANTLR paren wrap rules for C semicolons', function() {
  let editor;
  window.editor = (editor = new droplet.Editor(document.getElementById('test-main'), {
    mode: 'c',
    palette: []
...
asyncTest('Controller: ANTLR reparse rules', function() {
  let editor;
...
`
  ...
  var loopblock = function(head) {
    if (head === editor.session.tree.end) {
      start();
      return;
    }

    if ((head.type === 'blockStart') && (head.container.stringify().length > 0)) {
      const before = head.prev;

      const oldLocalSerialize = head.container.serialize();
      const oldSerialize = editor.session.tree.serialize();
      editor.reparse(head.container);
      const newSerialize = editor.session.tree.serialize();
      const newLocalSerialize = before.next.container.serialize();
    ...
    return setTimeout((() => loopblock(head.next)), 0);
  };
  return loopblock(editor.session.tree.start);
});
