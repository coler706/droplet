 ...
 */
const viewlib = require('./view');
const drawlib = require('./draw');
...
exports.setModeOptions = (language, options) => modeOptions[language] = options;
...
`;
  ...
  const replaceElement = function(element) {
    const language = element.getAttribute('droplet-lang');
    const mode = new (modes[language])(modeOptions[language]);

    const dropletDocument = mode.parse(element.innerText.trim());
    dropletDocument.opts.roundedSingletons = true;

    const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');

    const view = new viewlib.View(svg);
    const node = view.getViewNodeFor(dropletDocument);
    node.layout(0, 0);
  ...
  return (() => {
    const result = [];
    const iterable = document.getElementsByTagName('pre');
    for (let i = 0; i < iterable.length; i++) {
      const el = iterable[i];
      if (el.hasAttribute('droplet-lang')) {
        result.push(replaceElement(el));
      } else {
...
window.addEventListener('load', () => exports.highlight());
